...
/**
 * Get real-time schedule summary and availability for landing page (doctor_id = doctors.id).
 * Uses doctor_schedules for schedule text and optionally appointments_public for availability.
 */
function getDoctorRealtimeScheduleAndAvailability(PDO $db, int $doctor_id): array {
    $out = ['schedule' => 'By Appointment', 'availability' => 'Available', 'availability_color' => 'gray'];
    try {
        $stmt = $db->prepare("
    ...
    $bySlot = [];
    foreach ($rows as $r) {
        $key = (normalizeTime($r['start_time'] ?? '') . '-' . normalizeTime($r['end_time'] ?? ''));
        if (!isset($bySlot[$key])) {
            $bySlot[$key] = ['days' => [], 'start' => $r['start_time'], 'end' => $r['end_time']];
        }
        $bySlot[$key]['days'][] = (int) $r['day_of_week'];
    }
    $parts = [];
    foreach ($bySlot as $slot) {
        sort($slot['days']);
        $dayStr = formatScheduleDays($slot['days'], $dayNames);
        $startFmt = formatTime($slot['start'], 'g:i A');
        $endFmt = formatTime($slot['end'], 'g:i A');
        $parts[] = $dayStr . ' ' . $startFmt . ' – ' . $endFmt;
    }
    $out['schedule'] = implode('; ', $parts);

    $today = date('Y-m-d');
    $dayOfWeek = (int) date('w', strtotime($today));
    $hasToday = false;
    foreach ($rows as $r) {
        if ((int) $r['day_of_week'] === $dayOfWeek) {
            $hasToday = true;
            break;
        }
    }
    if (!$hasToday) {
        $out['availability'] = 'By schedule';
        $out['availability_color'] = 'gray';
        return $out;
    }
    try {
    ...
    if (in_array('doctor_id', $columns) && in_array('appointment_date', $columns)) {
        $statusCol = in_array('status', $columns);
        $sql = "SELECT COUNT(*) AS cnt FROM appointments_public WHERE doctor_id = :doctor_id AND appointment_date = :d";
        if ($statusCol) {
            $sql .= " AND (status IS NULL OR status NOT IN ('cancelled', 'rejected'))";
        }
        $stmt = $db->prepare($sql);
        $stmt->execute([':doctor_id' => $doctor_id, ':d' => $today]);
        $count = (int) $stmt->fetch(PDO::FETCH_ASSOC)['cnt'];
        $schedule = getDoctorScheduleForDate($db, $doctor_id, $today);
        $maxSlots = 0;
        if ($schedule && isset($schedule['start_time'], $schedule['end_time'], $schedule['slot_duration_minutes'])) {
            $dur = (int) $schedule['slot_duration_minutes'];
            if ($dur > 0) {
                $maxSlots = (int) ((strtotime($schedule['end_time']) - strtotime($schedule['start_time'])) / ($dur * 60));
            }
        }
        if ($maxSlots > 0 && $count >= $maxSlots) {
            $out['availability'] = 'Fully booked today';
            $out['availability_color'] = 'red';
        } elseif ($count > 0) {
            $out['availability'] = 'Limited today';
            $out['availability_color'] = 'yellow';
        } else {
            $out['availability'] = 'Available today';
            $out['availability_color'] = 'green';
        }
    } else {
        $out['availability'] = 'Available today';
        $out['availability_color'] = 'green';
...
function formatScheduleDays(array $days, array $dayNames): string {
    if (empty($days)) {
        return '';
    }
    $ranges = [];
    ...
    while ($i < count($days)) {
        $start = $days[$i];
        $end = $start;
        while ($i + 1 < count($days) && $days[$i + 1] === $end + 1) {
            $i++;
            $end = $days[$i];
        }
        if ($start === $end) {
            $ranges[] = $dayNames[$start] ?? (string) $start;
        } else {
            $ranges[] = ($dayNames[$start] ?? $start) . '–' . ($dayNames[$end] ?? $end);
        }
        $i++;
    }
