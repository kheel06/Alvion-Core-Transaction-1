...
try {
    // Check if wards and beds tables exist
    $check_wards = $db->query("SHOW TABLES LIKE 'wards'");
    $wards_exists = $check_wards->rowCount() > 0;
    $check_beds = $db->query("SHOW TABLES LIKE 'beds'");
    $beds_exists = $check_beds->rowCount() > 0;

    if ($wards_exists && $beds_exists) {
    ...
    if ($admissions_exists) {
        $trend_query = "SELECT 
            ...
            COUNT(*) as admissions,
            (SELECT COUNT(*) FROM admissions a2 WHERE a2.status = 'admitted' AND DATE(a2.admission_date) <= DATE(MAX(a.admission_date))) as total_inpatients
            FROM admissions a
    ...
    // Current occupancy summary (only if beds table exists)
    $current_occupancy = ['total_beds' => 0, 'occupied_beds' => 0, 'available_beds' => 0, 'maintenance_beds' => 0, 'overall_occupancy_rate' => 0];
    if ($beds_exists) {
        $current_occupancy_query = "SELECT 
            ...
            COUNT(CASE WHEN status = 'available' THEN 1 END) as available_beds,
            COUNT(CASE WHEN status = 'maintenance' THEN 1 END) as maintenance_beds,
            ROUND((COUNT(CASE WHEN status = 'occupied' THEN 1 END) / NULLIF(COUNT(*), 0)) * 100, 2) as overall_occupancy_rate
            FROM beds";
        $current_occupancy_stmt = $db->prepare($current_occupancy_query);
        $current_occupancy_stmt->execute();
        $current_occupancy = $current_occupancy_stmt->fetch() ?: $current_occupancy;
    }
...
<!-- Report Filters -->
<div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow rounded-lg mb-6">
    <div class="px-4 py-5 sm:p-6">
        <form method="GET" class="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div>
                <label for="start_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Start Date</label>
                <input type="date" name="start_date" id="start_date" value="<?php echo $start_date; ?>" 
                    class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary-500 focus:border-primary-500 bg-white dark:bg-gray-900/40 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400">
            </div>
            ...
            <div class="flex items-end">
                <button type="submit" 
                    class="w-full px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 focus:ring-offset-white dark:focus:ring-offset-gray-900">
                    Generate Report
                </button>
            </div>
        </form>
...
<div class="grid grid-cols-1 gap-6 mb-8 sm:grid-cols-2 lg:grid-cols-4">
    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 overflow-hidden shadow rounded-lg transition-colors">
        <div class="p-5">
            <div class="flex items-center">
                ...
                <div class="ml-5 w-0 flex-1">
                    <dl>
                        <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 truncate">Total Beds</dt>
                        <dd class="text-lg font-medium text-gray-900 dark:text-white"><?php echo $current_occupancy['total_beds'] ?? 0; ?></dd>
                    </dl>
                </div>
            </div>
        </div>
            ...
            <div class="flex items-center">
                ...
                <div class="ml-5 w-0 flex-1">
                    <dl>
                        ...
                        <dd class="text-lg font-medium text-gray-900 dark:text-white"><?php echo $current_occupancy['available_beds'] ?? 0; ?></dd>
                    </dl>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 overflow-hidden shadow rounded-lg transition-colors">
        <div class="p-5">
            <div class="flex items-center">
                ...
                <div class="ml-5 w-0 flex-1">
                    <dl>
                        <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 truncate">Occupied</dt>
                        <dd class="text-lg font-medium text-gray-900 dark:text-white"><?php echo $current_occupancy['occupied_beds'] ?? 0; ?></dd>
                    </dl>
                </div>
            </div>
        </div>
            ...
            <div class="flex items-center">
                ...
                <div class="ml-5 w-0 flex-1">
                    <dl>
                        ...
                        <dd class="text-lg font-medium text-gray-900 dark:text-white"><?php echo $current_occupancy['overall_occupancy_rate'] ?? 0; ?>%</dd>
                    </dl>
                </div>
            </div>
        </div>
...
<div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
    <!-- Ward-wise Occupancy -->
    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow rounded-lg transition-colors">
        ...
        <div class="px-4 py-5 sm:p-6">
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead>
                        <tr>
                            <th class="px-4 py-3 bg-gray-50 dark:bg-gray-700/50 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                                Ward
                            </th>
                            <th class="px-4 py-3 bg-gray-50 dark:bg-gray-700/50 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                                Capacity
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-4 whitespace-nowrap text-sm">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                        <?php echo $ward['occupancy_rate'] > 90 ? 'bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-200' : ''; ?>
                                        <?php echo $ward['occupancy_rate'] > 70 && $ward['occupancy_rate'] <= 90 ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/40 dark:text-yellow-200' : ''; ?>
                                        <?php echo $ward['occupancy_rate'] <= 70 ? 'bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-200' : ''; ?>">
        ...
        <div class="px-4 py-5 sm:p-6">
                ...
                <div class="space-y-4">
                    <?php foreach ($occupancy_trend as $day): ?>
                        ...
                        <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                            <div class="bg-primary-600 dark:bg-primary-500 h-2 rounded-full" 
            ...
            <?php else: ?>
                <div class="text-center py-4">
                    <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-gray-300 dark:text-gray-600 mx-auto mb-2"><path d="M3 3v18h18"></path><path d="m19 9-5 5-4-4-3 3"></path></svg>
...
<!-- Export Options -->
<div class="mt-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow rounded-lg transition-colors">
    <div class="px-4 py-5 sm:p-6">
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-2">
                ...
                <a href="?<?php echo http_build_query(array_merge($_GET, ['export' => 'excel'])); ?>" 
                   class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
