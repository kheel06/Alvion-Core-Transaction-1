...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $check_stmt = $db->prepare("SELECT id FROM roles WHERE role_name = :role_name AND id != :id");
            $check_stmt->bindParam(':role_name', $name);
            $check_stmt->bindParam(':id', $role_id);
            $check_stmt->execute();
            if ($check_stmt->fetch()) {
                $errors[] = "Role name already exists.";
            }
        } catch (PDOException $e) {
            $errors[] = "Error checking existing role: " . $e->getMessage();
        }
    }
        ...
        <form method="POST" class="space-y-6">
            <div>
                ...
                <input type="text" name="name" id="name" required
                    value="<?php echo htmlspecialchars($_POST['name'] ?? $role['role_name']); ?>"
